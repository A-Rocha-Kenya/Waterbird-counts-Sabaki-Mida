...
Counts were also performed separably in a northern and southern transects. We combines/not used these counts here. 
...
```{r}
path = "C:/Users/rnussba1/OneDrive/ARK/ARK - Science/Water Bird Count/"

counts <- read_xlsx(paste0(path,"Data/water_bird_count_data.xlsx"), sheet = "Main", col_types = c("text", "numeric","text", "text", "date", "date", "date", "text","text","text","text","text","text","text","text")) %>%
  mutate(site = ifelse(site=='Mida Creek (South)' | site=='Mida Creek (North)', 'Mida Creek', site)) %>% 
  mutate(site = ifelse(site=='Sabaki (South)' | site=='Sabaki (North)', 'Sabaki', site)) %>% 
  filter( site=='Sabaki' | site=='Mida Creek') %>% 
...
participants.list <- read_xlsx(paste0(path,"Data/water_bird_count_data.xlsx"), sheet = "Participant")
...
counts %>% 
  group_by(date, site) %>% 
  summarise(disctinct_startdate = n_distinct(start_time),
            disctinct_enddate = n_distinct(end_time),
...
### Assessement 3: Specie and count
...
species.list.recorded %>% kable() %>% kable_styling()
```
...
family.list.recorded %>% kable() %>% kable_styling()
...
```
The dataset includes `r species.list.recorded %>% filter(taxon_rank=="Species") %>% nrow()` unique species and `r species.list.recorded %>% filter(taxon_rank!="Species") %>% nrow()` subspecies, families, slashs, etc. belonging to `{r} nrow(family.list.recorded)` families. 
...
## Write to Excel
```{r}
write.csv(events, file = paste0(path,"data/events.csv"), 
          na = "", row.names = FALSE, fileEncoding = "UTF-8")

write.csv(occurences, file = paste0(path,"data/occurences"), 
          na = "", row.names = FALSE, fileEncoding = "UTF-8")
```
