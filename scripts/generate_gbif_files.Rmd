...
```{r}
path = "/Users/raphael/Library/CloudStorage/OneDrive-SharedLibraries-ARK/Science - Documents/02 Birds/01. Taxon-specific studies or projects/03. Waterbirds/"

countst <- read_xlsx(paste0(path,"02 Data/water_bird_count_data.xlsx"), sheet = "Main", col_types = c("text", "numeric","text", "text", "date", "date", "date", "text","text","text","text","text","text","text","text")) %>%
...
species.list <- read_xlsx(paste0(path,"02 Data/water_bird_count_data.xlsx"), sheet = "Species")

locations.list <- read_xlsx(paste0(path,"02 Data/water_bird_count_data.xlsx"), sheet = "Sites") %>% 
  filter( site_name=='Sabaki' | site_name=='Mida Creek')

participants.list <- read_xlsx(paste0(path,"02 Data/water_bird_count_data.xlsx"), sheet = "Participants")
...
geojson <- fromJSON(paste0(path,"Waterbird-counts-Sabaki-Mida/assets/locations.geojson"))

track = readOGR(paste0(path,"Waterbird-counts-Sabaki-Mida/assets/locations.geojson"),require_geomType = "wkbLineString", verbose=F) %>% spTransform( CRS("+init=epsg:4326"))
area = readOGR(paste0(path,"Waterbird-counts-Sabaki-Mida/assets/locations.geojson"),require_geomType = "wkbPolygon", verbose=F)%>% spTransform( CRS("+init=epsg:4326"))
...
png(paste0(path,"Waterbird-counts-Sabaki-Mida/assets/coverage_",s,".png"), width = 800, height = 400)
...
write.csv(events, file = paste0(path,"Waterbird-counts-Sabaki-Mida/dwc_files/events_",v,".csv"), 
          na = "", row.names = FALSE, fileEncoding = "UTF-8")

write.csv(occurences, file = paste0(path,"Waterbird-counts-Sabaki-Mida/dwc_files/occurences_",v,".csv"), 
          na = "", row.names = FALSE, fileEncoding = "UTF-8")
```
